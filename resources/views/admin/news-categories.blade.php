            ...
            @foreach ($categories as $category)
                <div>
                    ...
                    <form action="{{route('admin-news-categories-delete', $category->id)}}" method="post">
                        @csrf
                        @method('delete')
                        <input class="delete-btn" type="submit" value="DELETE" onclick="return confirm('ARE YOU SURE')">
            ...
            <div class="category-news">
                ...
                @foreach ($category->news as $item)
                    <div>
                        ...
                        <p>{{$item->content}}</p>
                        <div>
                            <a href="{{route('edit-news', $item->id)}}" onclick="return confirm('ARE YOU SURE TO LEAVE THE PAGE')">EDIT</a>
                            
                            <form action="{{route('delete-news', $item->id)}}" method="post">
                                @csrf
                                @method('delete')
                                
                                <input type="submit" value="DELETE" onclick="return confirm('ARE SURE TO DELETE')">
